...
def draw_me_8(img, x0, y0, r, color):
    d = 3 - 2 * r
    x, y = 0, r
    while y >= x:
...
def draw_me_4_dashed(img, x0, y0, r, color):
    d = 2 - 2 * r
    x, y = 0, r
    while y >= 0:
        # Координаты четырёх симметричных точек
        for dx, dy in [(x, y), (x, -y), (-x, y), (-x, -y)]:
            angle = math.degrees(math.atan2(dy, dx))
            if angle < 0:
                angle += 360
            # Проверяем сектор (30° нарисовано, 30° пропущено)
            if int(angle // 30) % 2 == 0:
...
def draw_line_outside_polygon(img, x0, y0, x1, y1, color, polygon):
    # Находим пересечения линии с многоугольником
    intersections = []
    ...
    # Добавляем начало и конец линии
    intersections.append((x0, y0))
    intersections.append((x1, y1))

    # Сортируем по положению вдоль линии
    intersections.sort(key=lambda p: (p[0] - x0)**2 + (p[1] - y0)**2)

    # Проходим по парам и рисуем только внешние участки
...
def draw_me_8_225(img, x0, y0, r, color):
    d = 3 - 2 * r
    x, y = 0, r
    while y >= x:
        img.putpixel((x0 + x, y0 + y), color)
...
def fill_texture_outside_circle(img, inters, texture, circle_center, circle_radius):
    ...
    cx, cy = circle_center
    r2 = circle_radius ** 2  # радиус в квадрате для проверки
...
draw_line(img, 60, 130, 100, 50, (255, 0, 0))
draw_line(img, 60, 130, 140, 130, (255, 0, 0))
draw_line(img, 100, 50, 140, 130, (255, 0, 0))

triangle = [(60, 130), (100, 50), (140, 130)]
draw_line_outside_polygon(img, 0, 6, 199, 110, (255, 0, 0), triangle)

draw_line_dashed(img, 56, 133, 100, 45, (255, 0, 0))
draw_line_dashed(img, 56, 133, 144, 133, (255, 0, 0))
draw_line_dashed(img, 100, 45, 144, 133, (255, 0, 0))

draw_me_8(img, 100, 100, 20, (255, 0, 0))
draw_me_4_dashed(img, 100, 100, 17, (255, 0, 0))
draw_me_8_225(img, 100, 100, 90, (255, 0, 0))
...
texture = Image.open(r"C:\Obsidian\Learn\Уник\5ый сем\3D Моделирование\rk1\avatar.jpg").convert("RGB")
